...
from xrc_utils.digishield import read_header, target_to_bits, get_targetDigishield, bits_to_target
...
def test_read_header():
    ...
    df = dump_blockchain_headers_file_to_df(read_header.DATA_PATH, bits_format="electrum")

    for h in [149585, 154990, 151877]:
        header = read_header(h)
        for key in header:
            assert header[key] == df.loc[h].to_dict()[key], f"Mismatch between read_header and parsed df at height = {h}"
